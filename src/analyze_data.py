...
class SubsystemSummary:
    ...
    classes: Counter
    functions: Counter
    fault_component_pairs: Counter
    component_fault_variety: Dict[str, int]
...
def summarize_subsystem(name: str) -> SubsystemSummary:
    ...
    fault_component_pairs = Counter(
        (row["Fault_Type"], row["Target_Component"]) for row in rows
    )

    component_fault_variety: Dict[str, set[str]] = {}
    for row in rows:
        component_fault_variety.setdefault(row["Target_Component"], set()).add(
            row["Fault_Type"]
        )
    ...
    return SubsystemSummary(
        ...
        classes=classes,
        functions=functions,
        fault_component_pairs=fault_component_pairs,
        component_fault_variety={k: len(v) for k, v in component_fault_variety.items()},
    )
...
def format_pair_counter_table(
    counter: Counter[Tuple[str, str]],
    limit: int | None = None,
    separator: str = " → ",
) -> str:
    items = counter.most_common(limit)
    lines = ["| Fault → Target | Count |", "| --- | ---: |"]
    lines.extend(
        f"| {fault}{separator}{target} | {count} |" for (fault, target), count in items
    )
    return "\n".join(lines)
...
def generate_visualizations(summaries: Sequence[SubsystemSummary]) -> List[Tuple[str, Path]]:
    ...
    for summary in summaries:
        ...
        figures.append(
            (
                f"{summary.name} top fault-type ↔ component pairs",
                plot_bar_chart(
                    [
                        (f"{fault} → {component}", count)
                        for (fault, component), count in summary.fault_component_pairs.most_common(10)
                    ],
                    title=f"{summary.name} fault-type to component links (top 10)",
                    xlabel="Fault type → Target component",
                    ylabel="Count",
                    filename=f"{summary.name.lower()}_fault_component_pairs.svg",
                    rotation=25,
                ),
            )
        )
        ...
        figures.append(
            (
                f"{summary.name} components with diverse fault types",
                plot_bar_chart(
                    sorted(
                        summary.component_fault_variety.items(),
                        key=lambda kv: kv[1],
                        reverse=True,
                    )[:10],
                    title=f"{summary.name} components by fault-type diversity (top 10)",
                    xlabel="Component",
                    ylabel="Distinct fault types",
                    filename=f"{summary.name.lower()}_component_diversity.svg",
                ),
            )
        )
    ...
    for summary in summaries:
        content.extend(
            [
                ...
                format_pair_counter_table(summary.fault_component_pairs, limit=5),
                "",
                "Components experiencing the widest variety of fault types:",
                format_counter_table(
                    Counter(summary.component_fault_variety),
                    limit=5,
                ),
                "",
            ]
